var _ = require('underscore');

module.exports = {

  objectify: function objectify(list, value) {
    if (list == null) return {};
    var result = {};
    for (var i = 0, length = list.length; i < length; i++) {
      if (value === undefined) value = true;
      result[list[i]] = value;
    }
    return result;
  },

  chunk: function(list, chunkSize) {
    if (!chunkSize) return list;

    var A = [];
    for (var i=0; i < list.length; i+=chunkSize) {
      A.push(list.slice(i, i+chunkSize));
    }
    return A;
  },

  bucketize: function bucketize(list /* groupings */) {
    var groupings = _.map(Array.prototype.splice.call(arguments, 1), function (g) {
      return _.isString(g) ? { by: g, as: 'children', name: 'name' } : g;
    });

    function groupToArray(obj, as, name) {
      as || (as = 'children');
      name || (name = 'name');
      return _.map(obj, function (items, key) {
        var r = {};
        r[name] = key;
        r[as] = items;
        return r;
      });
    }

    function _recursiveBucketize(values, groupings) {
      var firstGrouping = groupToArray(_.groupBy(values, groupings[0].by), groupings[0].as, groupings[0].name);
      if (groupings.length > 1) {
        _.each(firstGrouping, function (group) {
          group[groupings[0].as] = _recursiveBucketize(group[groupings[0].as], groupings.slice(1));
        });
      }

      return firstGrouping;
    }

    return _recursiveBucketize(list, groupings);
  }

};
